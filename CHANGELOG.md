...
- 978670c: Callout（dismissible なインライン告知バー）を追加した（#211）
...
- 6dc31ca: MediaCard を常に block レベルのボックスとして描くようにした（#207）

  `render` で `<a>` / `<button>` に差し替えたカードは、grid / flex の item にならない位置（ラッパー `<div>` の中など）に置くと `display: inline` のまま残っていた。inline のまま `padding` / `border` / `border-radius` / `background` が付くため、背景ボックスが行の切れ目で断片に割れ（実測 3 断片）、カードの面が消えたように見えていた。カバーの full-bleed（`-mx-4`）も親幅を基準にできずカード幅を超えてはみ出していた（実測 +32px）。
...
- 84fcfae: MessageItem に `continuation` prop を追加した。連続投稿(同一投稿者の続き)を詰めて描くための口で、true のとき:
...
- 93dcf66: SplitModal のナビにグループ見出し・上部スロット・高さの上書きを足す

  設定項目が増えたアプリ（insession-app #1870 のユーザー設定）で、ナビが一列に伸び続けて走査しづらくなる問題に対応する。

  - `SplitModalItem.group` — 連続する同じ文字列の項目が 1 つの束になり、束の先頭にだけ見出しが出る。見出しは `role="group"` + `aria-labelledby` で束と結び付く。広い画面のレールでも狭い画面のドリルダウン一覧でも出る。
  ...
  いずれも任意。`group` を渡さない既存の呼び出しは束が 1 つできるだけで、従来どおりフラットな一列のまま描画される。
...
- 9eee02f: fix(icon): `waving_hand` を Material Symbols Outlined の公式パスへ差し替え

  これまで `waving_hand` には Material Symbols / レガシー Material Icons のどれとも一致しない出所不明のパスが入っており、「指の分かれていない塊状の手」として描画されていた（公式にあるモーション弧 2 本も欠けていた）。Material Symbols Outlined (FILL@0 / wght@400 / GRAD@0 / opsz@24) の公式パスへ差し替え、あわせて `PATHS`（`0 0 24 24`）から `EXTRA_PATHS` + `CUSTOM_VIEWBOX`（`0 -960 960 960`）へ移設した。`IconName` は変わらないため消費側の変更は不要。

## 7.13.0

### Minor Changes

- 4108fb8: `EmojiPicker` に、消費側の既存実装を置き換えるのに必要だった振る舞いを足した（#193）。
...
- 981d0b7: 絵文字ピッカー（`EmojiPicker`）と `:` ショートコード補完（`EmojiSuggest`）を追加した（#190）。
...
- 1d4f927: MediaCard: タイトルのフォントサイズを text-base(14px) から text-sm(12px) へ一段下げた。カードが小さくなるモバイル幅で、タイトルだけが不釣り合いに大きく見えていたため。

## 7.11.1

### Patch Changes

- 09dc570: Tabs: 行幅に収まらないタブ列を横スクロールできるようにし、モバイル幅では文字を一段小さくした。
  ...
  あわせて、モバイル幅（sm 未満）のタブを `text-xs`（11px）/ `tracking-wider`（0.05em）にした。日本語ラベルは字数がそのまま幅になり 0.1em の字間が余計に幅を食うため、6 タブで 442px → 408px（約 8%）縮み、1 画面に入るタブが 1 つ増える。sm 以上は従来どおり `text-sm`（12px）/ `tracking-widest`（0.1em）。既に収まっているタブ列の見た目は sm 以上では変わらない。
...
- 743a254: Mention: `@` メンション候補サジェスト用のプリミティブを追加
...
- 43b228a: BottomSheet: 閉じるボタンをハンドル行の中に収めた。
  ...
  シートの地から 1 段だけ持ち上げる。⚠ 当たり判定は `::after` で 44px 角へ広げてあるので、
  見た目を小さくしてもタップ領域は従来（36px）より広い。ハンドルは行の中央にあり領域は重ならない。

- 43b228a: SplitModal: `asSheet` のとき自前の × を描かないようにした（× が 2 つ並ぶのを修正）。
...
- eb19b5d: feat(tabs): タブのラベルを一段小さくする
...
- 449f2c0: SideNav.Account のメニュー配置を `side` / `align` / `sideOffset` で指定できるようにした（#169）。既定は従来どおり `side="top"` / `align="start"` / `sideOffset` は Menu の既定値なので、既存の利用側の見た目は変わらない。`side` が縦方向（`top` / `bottom`）以外のときはトリガー幅への追従（`--anchor-width`）を適用しないので、レールの横へ出してもメニューがレール幅に固定されない。

## 7.7.0

### Minor Changes

- b360748: RingTimer: 中央の数字を出さないモード（`showValue`）・リングの太さ指定（`thickness`）・リング色の差し替え（`ringColor`）を追加
...
- 2a91cd8: Icon に 15 種を追加（code / content_copy / dark_mode / error_outline / folder / groups / how_to_vote / hub / light_mode / menu_book / publish / rocket_launch / sync / tune / visibility）。
...
- 65446c0: Icon に 10 種を追加（arrow_back / bolt / build / cloud_off / error / mail / refresh / route / search_off / verified）。
...
- 0ee925d: EmptyState を追加。アイコン + タイトル + ヒント + CTA でセクション単位の空状態を伝えるカード。insession-app の community 画面にローカル実装されていた CommunityEmptyState を DS へ引き上げたもの。一行で足りる文脈（キュー/履歴タブ・検索候補ドロップダウン）は従来どおり EmptyNote を使う。

## 7.3.0

### Minor Changes

- f2a5725: feat(drawer): 画面端からスライドインする Drawer プリミティブを追加
