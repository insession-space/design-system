...
export type MenuPopupProps = React.ComponentProps<typeof BaseMenu.Popup> & PopupSurfaceProps;

function MenuPopup({ className = '', padding = true, scroll = true, ...props }: MenuPopupProps) {
  return (
    <BaseMenu.Popup
      className={`${popupSurfaceClassName(padding, scroll)} ${className}`.trim()}
      {...props}
    />
  );
}
...
// bg-transparent を落とすだけにしないのは、DS が preflight を配っていないため
