...
// DS のテキストリンク（純粋 leaf UI）。#633。用途で3種に整理する:
//   inline  = 本文中リンク。下線あり(thin / offset 2px)、hover でコーラル + 下線強調。
//   subtle  = リスト脇の弱い誘導（旧 `text-accent text-sm` の置換先）。静止は色のみ、hover で下線。
...
// 下線・色・トランジションはトークン経由。arbitrary は 4/8px リズムに載らない値のみ（py-[7px] 等）。
const VARIANT: Record<LinkVariant, string> = {
  inline:
    'text-link underline decoration-1 underline-offset-2 decoration-link/45 cursor-pointer transition-[color,text-decoration-color] duration-(--dur-fast) hover:text-link-hover hover:decoration-link-hover',
  subtle:
    'text-link text-sm no-underline cursor-pointer transition-colors duration-(--dur-fast) hover:text-link-hover hover:underline hover:underline-offset-2',
